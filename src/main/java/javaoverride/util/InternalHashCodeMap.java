...
import javaoverride.util.AbstractMap.SimpleEntry;
import javaoverride.util.Map.Entry;

/**
 * A simple wrapper around JavaScriptObject to provide
 * {@link javaoverride.util.Map}-like semantics for any key type.
 * <p>
 * Implementation notes:
 * <p>
 * A key's hashCode is the index in backingMap which should contain that key.
 * Since several keys may have the same hash, each value in hashCodeMap is
 * actually an array containing all entries whose keys share the same hash.
 */
class InternalHashCodeMap<K, V> implements Iterable<Entry<K, V>> {
	...
	public java.util.Iterator<Entry<K, V>> iterator() {
		return IterableHelper.castIteratorToLang(new Iterator<Map.Entry<K, V>>() {
			...
			public void remove() {
				...
				// Unless we are in a new chain, all items have shifted so our itemIndex should
				// as well...
				if (itemIndex != 0) {
					itemIndex--;
				}
	...
	/**
	 * Returns hash code of the key as calculated by
	 * {@link AbstractHashMap#getHashCode(Object)} but also handles null keys as
	 * well.
	 */
